...
public class XMLGeneration {
	...
	private void writeSheet(XMLStreamWriter xmlWriter, short sheetIndex, Sheet sheet) throws XMLStreamException {
		...
		xmlWriter.writeAttribute("hidden", String.valueOf(poiWorkbook.isSheetHidden(sheetIndex)));
		xmlWriter.writeAttribute("very_hidden", String.valueOf(poiWorkbook.isSheetVeryHidden(sheetIndex)));
	...
	private void writeDataValidation(XMLStreamWriter xmlWriter, CellRangeAddress address, String formula)
	...
	private void writeColumns(XMLStreamWriter xmlWriter, Sheet sheet) throws XMLStreamException {
		...
		for (int x = firstCol - 1; x < lastCol; x++) {
			...
			xmlWriter.writeAttribute("column_alpha", String.valueOf(column_alpha(x)));
			xmlWriter.writeAttribute("width", String.valueOf(sheet.getColumnWidth(x)));
			xmlWriter.writeEndElement();
		}
		xmlWriter.writeEndElement();
	}

	private void writeRows(XMLStreamWriter xmlWriter, Sheet sheet) throws XMLStreamException {
		...
		for (int x = row.getFirstCellNum(); x <= row.getLastCellNum(); x++) {
			...
			if (cell != null) {
				...
				if (info.value != null) {
					...
					xmlWriter.writeAttribute("row", String.valueOf(row.getRowNum() + 1));
	 ...
	 * 
	 * @param original
	 * @return the same String but with XML 1.0 invalid characters (like form
	 *         feed) removed
	 */
	private String xml10Characters(String original) {
		// TODO: this would be better incorporating into an custom version of
